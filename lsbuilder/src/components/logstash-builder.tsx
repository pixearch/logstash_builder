...
const PLUGIN_DEFINITIONS = {
  ...
  filter: {
    ...
    geoip: {
      ...
      fields:
        '["city_name", "continent_code", "country_code2", "country_name", "ip", "latitude", "longitude", "postal_code", "region_name", "timezone"]',
      ecs_compatibility: '"disabled"',
      source: '"clientip"',
...
const ConnectionLines = ({ activeField, dependencies, containerRef }) => {
  ...
  producers.forEach((pid) => {
    ...
    targets.forEach((t) => {
      ...
      lines.push(<circle key={`joint-${pid}-${t.id}`} cx={busX} cy={t.pos.y} r="2" fill="#6366f1" />)
    })

    // Start Dot at Producer
    lines.push(<circle key={`start-dot-${pid}`} cx={start.x} cy={start.y} r="3" fill="#10b981" />)
  })
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center gap-2 p-2 ${isContainer ? "border-b border-slate-800" : ""}`}>
        ...
        {node.condition && (
          <span className="text-xs font-mono text-yellow-500 truncate max-w-[200px]">{node.condition}</span>
        )}
...
export default function LogstashBuilder() {
  ...
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false) // Added state for delete confirmation dialog
  const [nodeToDelete, setNodeToDelete] = useState(null) // Added state to store node ID for deletion
  ...
  const handleDrop = (e) => {
    ...
    try {
      ...
      if (isPlugin && rootSection && itemCategory !== rootSection) {
        setErrorToast(
          `Error: Cannot place ${itemCategory.toUpperCase()} plugin into ${rootSection.toUpperCase()} section.`,
        )
      ...
      if (draggedItem.type === "NEW") {
        ...
        nodeToInsert = {
          id: generateId(),
          type: ["if", "else", "else if"].includes(draggedItem.name)
            ? "conditional"
            : draggedItem.category === "comment"
              ? "comment"
              : "plugin",
        ...
        <div key={key} className="group transition-all duration-200 mb-4">
          ...
          <div className={opacityClass}>
            {isMatch ? (
              ...
              <div
                className={`space-y-2 bg-slate-800/50 p-2 rounded border ${isActive ? "border-slate-700" : "border-slate-800"}`}
              >
                <div>
  ...
  return (
    <div className="flex h-screen w-full bg-slate-950 text-slate-200 overflow-hidden font-sans select-none">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-8">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-2xl rounded-lg shadow-2xl flex flex-col h-[60vh]">
            <div className="flex items-center justify-between p-3 border-b border-slate-700 bg-slate-800">
              <span className="font-mono text-sm text-slate-300 font-bold uppercase">
                Editing: {expandedProp.title}
              </span>
              <button onClick={() => setExpandedProp(null)}>
            ...
            <div className="p-3 border-t border-slate-700 flex justify-end gap-2">
              <button
                onClick={() => setExpandedProp(null)}
                className="px-4 py-1.5 rounded text-sm text-slate-400 hover:text-white"
              >
                Cancel
              </button>
              <button
                ...
                <div className="space-y-1 pl-2 border-l border-slate-800 ml-2">
                  {cat.items.map((item) => (
                    <div
                      ...
                      onDoubleClick={() => handleDoubleClick(item, cat.category)} // Added double-click handler
      ...
      <div className="flex-1 flex flex-col relative bg-[#0b1120]" id="canvas-area">
        ...
        >
          {showInsights && (
            <ConnectionLines activeField={activeField} dependencies={dependencies} containerRef={scrollContainerRef} />
          )}
          ...
          {showInsights && (
            <div className="absolute top-4 right-4 w-48 bg-slate-900/90 border border-slate-700 rounded p-2 z-30 shadow-2xl backdrop-blur">
              ...
              {Object.keys(dependencies).length === 0 && (
                <div className="text-xs text-slate-600 italic">No fields detected</div>
              )}
      ...
      {selectedNode ? ( // Only show right sidebar if a node is selected
        <div className="w-80 bg-slate-900 border-l border-slate-800 flex flex-col z-20 custom-scrollbar overflow-y-auto">
        ...
        </div> // If no node is selected, show placeholder in the right sidebar area
      ) : (
        <div className="w-80 bg-slate-900 border-l border-slate-800 flex flex-col z-20 items-center justify-center text-slate-600 p-8 text-center">
