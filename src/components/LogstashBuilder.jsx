...
const PLUGIN_DEFINITIONS = {
  // ... (keeping all existing plugin definitions)
  input: {
    azure_event_hubs: {
  ...
  filter: {
    ...
    alter: {
      coalesce: '[ "field", "value1", "value2" ]',
      condrewrite: '[ "field", "expected_value", "new_value" ]',
      condrewriteother: '[ "field", "expected_value", "target_field", "new_value" ]'
    },
    bytes: {
      source: '"bytes_field"',
      target: '"bytes_int"',
    ...
    cidr: {
      ...
      network_path: '"/etc/logstash/networks.txt"',
      refresh_interval: '600',
      separator: '","',
      add_tag: '["_cidr_matched"]'
    },
    cipher: {
    ...
    clone: {
      clones: '["cloned_type_1", "cloned_type_2"]',
      ecs_compatibility: '"disabled"',
      add_tag: '["cloned"]'
    },
    csv: {
    ...
    dissect: { 
      mapping: '{ "message" => "%{ts} %{+ts} %{src} %{} %{prog}[%{pid}]: %{msg}" }',
    ...
    dns: {
      ...
      resolve: '["source_field"]',
      reverse: '["source_field"]',
      tag_on_timeout: '["_dnstimeout"]',
      timeout: '0.5'
    },
    drop: { percentage: '100' },
    elapsed: {
      start_tag: '"task_start"',
      end_tag: '"task_end"',
    ...
    elastic_integration: {
      ...
      cloud_auth: '""',
      cloud_id: '""',
      geoip_database_directory: '"/usr/share/logstash/geoip"',
      hosts: '["localhost:9200"]',
      password: '""',
    ...
    geoip: {
      ...
      fields: '["city_name", "continent_code", "country_code2", "country_name", "ip", "latitude", "longitude", "postal_code", "region_name", "timezone"]',
      ecs_compatibility: '"disabled"',
      source: '"clientip"',
    ...
    grok: { 
      ...
      match: { field: 'message', patterns: ['%{COMBINEDAPACHELOG}'] },
    ...
    jdbc_static: {
      ...
      jdbc_password: '""',
      jdbc_user: '""',
      tag_on_default_use: '["_jdbcstaticdefaults"]',
      tag_on_failure: '["_jdbcstaticfailure"]',
    ...
    jdbc_streaming: {
      ...
      prepared_statement_bind_values: '[]',
      prepared_statement_name: '""',
      prepared_statement_warn_on_constant_usage: 'true',
      sequel_opts: '{}',
      statement: '"SELECT * FROM table WHERE id = :id"',
      tag_on_default_use: '["_jdbcstreamingdefaults"]',
      tag_on_failure: '["_jdbcstreamingfailure"]',
      target: '"jdbc_data"',
      use_cache: 'true',
      use_prepared_statements: 'false'
    },
    ...
    kv: {
      ...
      trim_value: '""',
      value_split: '"="',
      value_split_pattern: 'null',
      whitespace: '"strict"'
    },
    memcached: {
      hosts: '["localhost:11211"]',
      namespace: '"logstash"',
      get: '{ "key" => "target_field" }',
      set: '{ "source_field" => "key" }',
      tag_on_failure: '["_memcached_failure"]',
      ttl: '0'
    },
    metricize: {
    ...
    ruby: {
      code: '"event.set(\'new_field\', \'some_value\')"',
      init: '""',
      path: '""',
      script_params: '{}',
      tag_on_exception: '["_rubyexception"]',
      tag_with_exception_message: 'false'
    },
    sleep: {
      every: '1',
      replay: 'false',
    ...
    translate: {
      ...
      source: '"message"',
      target: '"translation"',
      yaml_dictionary_code_point_limit: '134217728',
      yaml_load_strategy: '"standard"'
    },
    truncate: {
      fields: '"message"',
      length_bytes: '10'
...
const getRootSectionName = (nodes, nodeId) => {
  const findRoot = (list, targetId, currentRoot) => {
...
const generateConfigString = (nodes, depth = 0) => {
  let output = '';
...
// --- DEPENDENCY ANALYZER ---
const analyzeDependencies = (nodes) => {
  const fieldMap = {}; 
  ...
  const register = (field, nodeId, type) => {
  ...
  const findInterpolatedFields = (str, nodeId) => {
  ...
  const traverse = (list) => {
    list.forEach(node => {
      ...
      if (node.properties) {
        ...
        Object.entries(node.properties).forEach(([key, val]) => {
           // Skip dissect mapping in generic pass to prevent misidentifying producers as consumers
        ...
        if (node.name === 'dissect' && node.properties.mapping) {
           ...
           // 1. Consumers: Keys in the mapping hash "field" => ...
           ...
           // 2. Producers: Fields in the pattern %{field}
           // Matches %{field}, %{+field} (append), but ignores %{?field} (skip)
           ...
           while ((targetMatch = targetRegex.exec(mappingStr)) !== null) {
               ...
               // Register as producer unless it is a named skip (?)
               if (modifier !== '?') {
...
const PluginIcon = ({ name, className = "w-4 h-4" }) => {
...
// --- IMPROVED CONNECTION VISUALIZER ---
const ConnectionLines = ({ activeField, dependencies, containerRef }) => {
  ...
  const getAnchor = (id) => {
    ...
    // Anchor Point: LEFT edge, vertically centered
    // IMPORTANT: Include scrollTop/scrollLeft to properly map to the scrolling SVG container
    return { 
  ...
  // Calculate Bus Line Position (left of the left-most element involved)
  let minX = Infinity;
  ...
  // If we can't find positions, abort
  if (minX === Infinity) return null;

  const busX = Math.max(10, minX - 30); // 30px buffer to the left
  ...
  producers.forEach(pid => {
    ...
    const targets = consumers
      .map(cid => ({ id: cid, pos: getAnchor(cid) }))
      .filter(t => t.pos && t.pos.y !== start.y)
      .sort((a, b) => a.pos.y - b.pos.y);

    if (targets.length === 0) return;

    // 1. Draw Producer Leg (Start -> Left to Bus)
    lines.push(
        <path
            key={`producer-leg-${pid}`}
            d={`M ${start.x} ${start.y} H ${busX}`}
            fill="none"
            stroke="#6366f1" 
            strokeWidth="2"
    ...
    // 3. Draw Consumer Legs (Bus -> Right to Consumer)
    targets.forEach(t => {
        lines.push(
            <path
                key={`consumer-leg-${pid}-${t.id}`}
                d={`M ${busX} ${t.pos.y} H ${t.pos.x - 6}`} // Stop before block for arrow
                fill="none"
                stroke="#6366f1"
                strokeWidth="2"
...
const PipelineNode = ({ node, selectedId, onSelect, onDrop, onDragStart, onDragOverNode, dropTarget, depth = 0, activeField, dependencies }) => {
  ...
  // 2. Insights Highlighting
  // IMPORTANT: Only apply if activeField is passed (Insights mode is ON)
  if (activeField && dependencies && dependencies[activeField]) {
      const { producers, consumers } = dependencies[activeField];
  ...
  return (
    <div 
      ref={nodeRef}
      id={`node-${node.id}`}
      className={`relative mb-2 rounded border ${borderColor} ${bgColor} ${shadowClass} transition-all duration-200`}
    ...
    >
      ...
      <div className={`flex items-center gap-2 p-2 ${isContainer ? 'border-b border-slate-800' : ''}`}>
        ...
        <span className={`text-sm font-mono font-bold ${isSelected ? 'text-white' : 'text-slate-300'}`}>
          {node.name}
        </span>
        {node.condition && <span className="text-xs font-mono text-yellow-500 truncate max-w-[200px]">{node.condition}</span>}
        {isSection && <span className="text-xs text-slate-600">{'{'}</span>}
      </div>
      ...
      {isSection && <div className="px-2 pb-1 text-xs text-slate-600">{'}'}</div>}
        ...
        <div className="px-3 pb-2 space-y-0.5">
          {Object.entries(node.properties || {}).slice(0, 3).map(([k, v]) => (
             <div key={k} className="text-[10px] flex gap-2 font-mono">
               ...
               <span className="text-green-400/80 truncate">{typeof v === 'object' ? JSON.stringify(v) : v}</span>
             </div>
          ))}
        </div>
      )}
...
export default function LogstashBuilder() {
  ...
  const [showDeleteConfirm, setShowDeleteConfirm] = useState(false); // NEW: Delete Confirm State
  const [nodeToDelete, setNodeToDelete] = useState(null); // NEW: Node tracked for deletion
  ...
  const expandAll = () => setOpenSections({
    input: true, filter: true, output: true, logic: true, comment: true
  });
  ...
  // Ref for the scrollable container to ensure arrow coordinates match scroll position
  ...
  // Derived state for library items
  ...
  const handleDoubleClick = (item, category) => {
    // 1. Determine Target Root Section
    let targetSectionId = '';
    if (category === 'input') targetSectionId = 'root-input';
    else if (category === 'output') targetSectionId = 'root-output';
    else targetSectionId = 'root-filter'; // Default filter, logic, comment to filter section
    ...
    const targetSection = newNodes.find(n => n.id === targetSectionId);
  ...
  const handleDragStart = (e, item, existingId = null) => {
    e.stopPropagation();
  ...
  const handleDragOverNode = (id, pos) => {
     if (!dropTarget || dropTarget.id !== id || dropTarget.pos !== pos) {
  ...
  const handleDrop = (e) => {
    ...
    // FIX: Prevent dropping on self which causes deletion
  ...
  const handleUpdateProp = (key, val, replaceAll = false) => {
    ...
    if (key === 'condition' || key === 'text') {
      ...
      if (replaceAll) {
        ...
        node.properties = { ...node.properties, [key]: val };
      }
    }
    setNodes(newNodes);
  };

  // --- NEW: Smart Delete Logic ---
  const handleRequestDelete = () => {
    if (!selectedNode) return;
    
    let isDirty = false;

    // Check 1: Container with children?
    if (selectedNode.children && selectedNode.children.length > 0) {
        isDirty = true;
    }
    // Check 2: Comment with text?
    else if (selectedNode.type === 'comment' && selectedNode.text && selectedNode.text.length > 0) {
        isDirty = true;
    }
    // Check 3: Plugin with modifications?
    else if (selectedNode.type === 'plugin') {
        const defs = PLUGIN_DEFINITIONS[selectedNode.category]?.[selectedNode.name] || {};
        const required = REQUIRED_FIELDS[selectedNode.category]?.[selectedNode.name] || [];
        const initialProps = {};
        ...
        // Compare JSON strings of current props vs initial default props
        if (JSON.stringify(selectedNode.properties) !== JSON.stringify(initialProps)) {
            isDirty = true;
        }
    }
    ...
    if (isDirty) {
        ...
        setNodeToDelete(selectedId);
        setShowDeleteConfirm(true);
    } else {
        // If clean (default/empty), delete immediately
  ...
  const renderPropertiesPanel = () => {
     ...
     // 4. Identify Common Keys
     const commonKeys = COMMON_KEYS[category] || [];

     // 5. Filter into Specific vs Common
     const specificKeys = allKeys.filter(k => !commonKeys.includes(k));
     // We sort specific keys alphabetically, common keys use predefined order
     specificKeys.sort();
     
     // Only show common keys that actually exist in definitions or props
     const activeCommonKeys = commonKeys; 

     const renderField = (key, isCommon = false) => {
        const isMatch = (key === 'match' && selectedNode.name === 'grok');
        const isActive = currentProps.hasOwnProperty(key);
        
        // Value Resolution: User Config -> Specific Plugin Def -> Common Default
        let val;
        if (isActive) {
           val = currentProps[key];
        } else {
           val = defaultDefs[key] !== undefined ? defaultDefs[key] : COMMON_DEFS[key];
        }
                                ...
                                <div key={idx} className="flex gap-1 items-center">
                                   ...
                                   {isActive && (
                                     <button 
                                        onClick={() => {
                                            if(val.patterns.length > 1) {
                                                const newPats = val.patterns.filter((_, i) => i !== idx);
                                                handleUpdateProp('match', { ...val, patterns: newPats });
                                            }
                                        }}
                     ...
                     {isActive && (
                       <button 
                         onClick={() => setExpandedProp({
                             title: key,
                             value: val || '',
                             onSave: (newVal) => handleUpdateProp(key, newVal)
                         })}
         ...
         <div className="pt-4 border-t border-slate-800 mt-4">
           ...
           <div className="flex gap-2">
             <input id="new-prop" placeholder="key_name" className="flex-1 bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-white" />
              <button 
               onClick={() => {
                 const key = document.getElementById('new-prop').value;
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-8">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-sm rounded-lg shadow-2xl p-6 text-center">
            ...
            <p className="text-slate-400 mb-6 text-xs">
              This block has customizations that will be lost.
            </p>
            <div className="flex justify-center gap-3">
              <button 
                ...
                className="px-4 py-2 text-slate-300 hover:text-white font-medium hover:bg-slate-800 rounded transition-colors text-xs"
              >
                Cancel
              </button>
              <button 
                onClick={() => performDelete(nodeToDelete)}
                className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white font-bold rounded shadow-lg transition-colors flex items-center gap-2 text-xs"
              >
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-8">
          <div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-lg shadow-2xl p-6 text-center">
            ...
            <p className="text-slate-400 mb-6 text-sm">
              This will completely clear your current configuration. This action cannot be undone.
            </p>
            <div className="flex justify-center gap-3">
              <button 
                onClick={() => setShowResetConfirm(false)}
                className="px-4 py-2 text-slate-300 hover:text-white font-medium hover:bg-slate-800 rounded transition-colors"
              >
                Cancel
              </button>
              <button 
                onClick={() => {
                  setNodes(JSON.parse(JSON.stringify(INITIAL_STATE)));
                  setSelectedId(null);
                  setActiveField(null);
                  setShowResetConfirm(false);
                }}
                className="px-4 py-2 bg-red-600 hover:bg-red-500 text-white font-bold rounded shadow-lg transition-colors flex items-center gap-2"
              >
      ...
      {expandedProp && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-8">
           <div className="bg-slate-900 border border-slate-700 w-full max-w-2xl rounded-lg shadow-2xl flex flex-col h-[60vh]">
              ...
              <textarea 
                 className="flex-1 bg-black text-green-300 font-mono p-4 resize-none outline-none text-sm"
              ...
              <div className="p-3 border-t border-slate-700 flex justify-end gap-2">
                 ...
                 <button 
                   onClick={() => {
                     expandedProp.onSave(expandedProp.value);
                     setExpandedProp(null);
                   }}
                   className="bg-sky-600 text-white px-4 py-1.5 rounded text-sm hover:bg-sky-500 flex items-center gap-2"
                 >
      ...
      <div className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col z-20">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
             ...
             <h1 className="font-bold tracking-tight">LS Builder</h1>
          </div>
          <div className="flex gap-1">
             <button 
               onClick={expandAll} 
        ...
        <div className="p-2 border-b border-slate-800 sticky top-0 bg-slate-900 z-10">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-slate-800/50 border border-slate-700 rounded pl-7 pr-7 py-1.5 text-xs text-slate-300 focus:border-sky-500 outline-none transition-colors"
              value={searchQuery}
          ...
          {filteredLibraryItems.map((cat) => (
            <div key={cat.category}>
              <button 
                ...
                className="flex items-center w-full text-left gap-1 text-xs font-bold text-slate-500 uppercase px-2 mb-2 hover:text-slate-300 transition-colors"
              >
        ...
        <div className="h-14 border-b border-slate-800 bg-slate-900/90 backdrop-blur flex items-center justify-between px-6 z-10">
          <div className="flex gap-2">
             <button 
               onClick={() => {
                 if (!showInsights) {
                    setShowConfig(false); // Turn off config if turning on insights
                 } else {
                    setActiveField(null); // Clear selection when exiting
                 }
                 setShowInsights(!showInsights);
               }}
               className={`flex items-center gap-2 px-3 py-1.5 rounded text-xs font-bold border transition-all ${showInsights ? 'bg-sky-500/10 border-sky-500 text-sky-400' : 'border-slate-700 text-slate-400 hover:text-slate-200'}`}
             >
               <Share2 className="w-3 h-3" /> Insights
             </button>
             <button 
               onClick={() => {
                 if (!showConfig) {
                    setShowInsights(false); // Turn off insights if turning on config
                    setActiveField(null);
                 }
                 setShowConfig(!showConfig);
               }}
               className={`flex items-center gap-2 px-3 py-1.5 rounded text-xs font-bold border transition-all ${showConfig ? 'bg-green-500/10 border-green-500 text-green-400' : 'border-slate-700 text-slate-400 hover:text-slate-200'}`}
             >
               <Code className="w-3 h-3" /> {showConfig ? 'Hide Config' : 'View Config'}
             </button>
          </div>
          <div className="flex gap-2">
             <button 
             ...
             <button 
                ...
                className="flex items-center gap-2 bg-sky-600 hover:bg-sky-500 text-white px-4 py-1.5 rounded text-xs font-bold shadow-lg shadow-sky-500/20"
             >
        ...
        >
             ...
             <div className="absolute top-4 right-4 w-48 bg-slate-900/90 border border-slate-700 rounded p-2 z-30 shadow-2xl backdrop-blur">
               ...
               {Object.keys(dependencies).map(field => (
                 <button 
                   ...
                   className={`w-full text-left text-xs px-2 py-1 rounded mb-1 flex justify-between items-center ${activeField === field ? 'bg-sky-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}
                 >
           ...
           {showConfig ? (
             <div className="relative h-full">
               <button
                 onClick={handleCopy}
                 className="absolute top-4 right-4 bg-slate-700 hover:bg-slate-600 text-white p-2 rounded shadow-lg z-10 transition-colors border border-slate-600"
                 title="Copy to Clipboard"
               >
                 {copySuccess ? <Check className="w-4 h-4 text-green-400" /> : <Copy className="w-4 h-4" />}
               </button>
               <pre className="font-mono text-xs text-green-400 bg-black/50 p-6 rounded border border-slate-800 h-full overflow-auto">
                 {generateConfigString(nodes)}
               </pre>
             </div>
           ) : (
               ...
               {nodes.map(section => (
                 ...
                 >
                   <PipelineNode 
                     ...
                     dropTarget={dropTarget}
                     activeField={showInsights ? activeField : null} // Force reset when insights is off
                     dependencies={dependencies}
                   />
                 </div>
               ))}
      ...
      <div className="w-72 bg-slate-900 border-l border-slate-800 flex flex-col z-20">
        <div className="p-4 border-b border-slate-800">
          <h2 className="font-bold flex items-center gap-2">
            <Settings className="w-4 h-4 text-sky-500" /> Properties
          </h2>
        </div>
        ...
        {selectedNode ? (
          <div className="flex-1 overflow-y-auto p-4">
             ...
             {(selectedNode.type === 'conditional' || selectedNode.condition !== undefined) && (
               <div className="mb-4">
                 ...
                 <div className="flex gap-1 mt-1">
                   <input 
                     className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-yellow-500 font-mono focus:border-sky-500 outline-none"
                     value={selectedNode.condition || ''}
                     onChange={(e) => handleUpdateProp('condition', e.target.value)}
                   ...
                   <button 
                     onClick={() => setExpandedProp({ 
                         title: 'Condition', 
                         value: selectedNode.condition || '',
                         onSave: (val) => handleUpdateProp('condition', val)
                     })}
                     className="bg-slate-800 border border-slate-700 hover:bg-slate-700 text-slate-400 px-2 rounded"
                   >
                     <Maximize2 className="w-3 h-3" />
             ...
             {selectedNode.type === 'comment' && (
               <div className="mb-4">
                 ...
                 <div className="flex gap-1 mt-1">
                   <textarea 
                     className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-sm text-slate-300 focus:border-sky-500 outline-none h-20"
                   ...
                   <button 
                     onClick={() => setExpandedProp({
                        title: 'Comment Text',
                        value: selectedNode.text || '',
                        onSave: (val) => handleUpdateProp('text', val)
                     })}
                     className="h-8 bg-slate-800 border border-slate-700 hover:bg-slate-700 text-slate-400 px-2 rounded"
                   >
                     <Maximize2 className="w-3 h-3" />
                   </button>
                 </div>
             ...
             {selectedNode.type === 'plugin' && selectedNode.properties && (
                 renderPropertiesPanel()
             )}

             <button 
               onClick={handleRequestDelete}
               className="w-full mt-8 bg-red-900/30 border border-red-900 text-red-400 py-2 rounded text-sm hover:bg-red-900/50 flex items-center justify-center gap-2"
             >
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-600 p-8 text-center">
            ...
            <p className="text-sm">Select a block in the pipeline to edit properties.</p>
          </div>
        )}
      </div>
    </div>
