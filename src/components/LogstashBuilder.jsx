...
const PipelineNode = ({
  ...
  activeField,
  dependencies,
  onRequestDelete, // Added prop for delete handler
}) => {
        ...
        <div className={`p-2 min-h-[40px] ${node.children.length === 0 ? "bg-slate-800/20" : ""}`}>
          ...
          {node.children.map((child) => (
            <PipelineNode
              ...
              onRequestDelete={onRequestDelete} // Pass delete handler to children
            />
          ))}
        </div>
      )}
...
export default function LogstashBuilder() {
  ...
  const handleRequestDelete = (nodeId = null) => {
    ...
    else if (nodeToCheck.type === "comment" && nodeToCheck.text && nodeToCheck.text.length > 0) {
      isDirty = true
    }
    // Check 3: Plugin with modifications?
      ...
      if (JSON.stringify(nodeToCheck.properties) !== JSON.stringify(initialProps)) {
        isDirty = true
      }
    }
              ...
              {nodes.map((section) => (
                ...
                >
                  <PipelineNode
                    ...
                    dependencies={dependencies}
                    onRequestDelete={handleRequestDelete} // Pass delete handler
                  />
                </div>
              ))}
            </div>
